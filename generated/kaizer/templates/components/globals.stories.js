...
const iconsArray = (icons) => {
  let iconString = '';
  if (window.drupalSettings.kaizerSvgSprite) {
    iconString +=
          ...
          <div style="height: 100%; display: flex; flex-direction: column; justify-content: center; gap: 4px;">
            <div>
              <svg aria-hidden="true" style="width: 16px; height: 16px; vertical-align: top;">
                <use xlink:href="${
                  window.drupalSettings.kaizerSvgSprite
...
export const IconsList = {
  render: () => {
    ...
    useEffect(() => {
      fetch(window.drupalSettings.kaizerSvgSprite)
...
export const Breakpoints = {
  render: () => {
    ...
    for (const [key, value] of Object.entries(
      window.drupalSettings.kaizerBreakpoints,
    )) {
