    ...
    attach: (context) => {
      once('wrapper-as-link', '[wrapper-as-link-container]', context).forEach(
        (el) => {
          ...
          if (link) {
            behaviors.kaizerWrapperAsLink.handler(el, link);
          }
        },
      );
    },
    handler: (el, link) => {
      ...
      if (isAjaxLink) {
        el.addEventListener('mousedown', (e) => {
          behaviors.kaizerWrapperAsLink.fakeLinkEvents(el, link, e, isAjaxLink);
        });
      } else {
        el.setAttribute(
          'aria-label',
        ...
        el.addEventListener('click', (e) => {
          behaviors.kaizerWrapperAsLink.fakeLinkEvents(el, link, e);
        });
      }
