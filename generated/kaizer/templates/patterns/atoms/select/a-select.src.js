...
(({ behaviors }) => {
  behaviors.kaizerAtomSelect = {
    attach: (context, settings) => {
      if (!Choices) {
        return;
      }
      once('a-select', '.a-select', context).forEach((el) => {
        behaviors.kaizerAtomSelect.handler(el, settings);
      });
    },
    handler: (el, settings) => {
          ...
          callbackOnCreateTemplates: (template) => {
            return {
              item: ({ classNames }, data) => {
                return template(`
                  <div
                    class="a-select__element a-select__js-preview-item ${
                      classNames.item
                    } ${
                  data.highlighted
                  ...
                  >${
                    data.label
                  }<svg class="a-select__chevron" aria-hidden="true">
                    <use xlink:href="${settings.kaizerSvgSprite}#svg-add"></use>
                  </svg></div>
                `);
              },
              choice: ({ classNames }, data) => {
                return template(`
                  <div class="${
                    !data.label ? 'a-select__js-select-item--default-value' : ''
