...
(({ behaviors }, $) => {
  behaviors.myCustomThemeHelperWrapperAsLink = {
    attach: (context) => {
      once('wrapper-as-link', '[wrapper-as-link-container]', context).forEach(
        (el) => {
          ...
          if (link) {
            behaviors.myCustomThemeHelperWrapperAsLink.handler(el, link);
          }
        },
      );
    },
    handler: (el, link) => {
      ...
      if (isAjaxLink) {
        el.addEventListener('mousedown', (e) => {
          behaviors.myCustomThemeHelperWrapperAsLink.fakeLinkEvents(
            el,
            link,
            e,
            isAjaxLink,
        ...
        el.addEventListener('click', (e) => {
          behaviors.myCustomThemeHelperWrapperAsLink.fakeLinkEvents(el, link, e);
        });
      }
