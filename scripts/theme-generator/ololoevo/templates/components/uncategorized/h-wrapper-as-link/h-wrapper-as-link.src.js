...
(({ behaviors }, $) => {
  behaviors.ololoevoHelperWrapperAsLink = {
    attach: (context) => {
      once('wrapper-as-link', '[wrapper-as-link-container]', context).forEach(
        (el) => {
          ...
          if (link) {
            behaviors.ololoevoHelperWrapperAsLink.handler(el, link);
          }
        },
      );
    },
    handler: (el, link) => {
      ...
      if (isAjaxLink) {
        el.addEventListener('mousedown', (e) => {
          behaviors.ololoevoHelperWrapperAsLink.fakeLinkEvents(
            el,
            link,
            e,
            isAjaxLink,
        ...
        el.addEventListener('click', (e) => {
          behaviors.ololoevoHelperWrapperAsLink.fakeLinkEvents(el, link, e);
        });
      }
