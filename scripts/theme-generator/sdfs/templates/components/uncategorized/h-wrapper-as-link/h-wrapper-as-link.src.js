...
(({ behaviors }, $) => {
  behaviors.sdfsHelperWrapperAsLink = {
    attach: (context) => {
      once('wrapper-as-link', '[wrapper-as-link-container]', context).forEach(
        (el) => {
          ...
          if (link) {
            behaviors.sdfsHelperWrapperAsLink.handler(el, link);
          }
        },
      );
    },
    handler: (el, link) => {
      ...
      if (isAjaxLink) {
        el.addEventListener('mousedown', (e) => {
          behaviors.sdfsHelperWrapperAsLink.fakeLinkEvents(
            el,
            link,
            e,
            isAjaxLink,
        ...
        el.addEventListener('click', (e) => {
          behaviors.sdfsHelperWrapperAsLink.fakeLinkEvents(el, link, e);
        });
      }
