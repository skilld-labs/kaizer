...
(({ behaviors }, Splide) => {
  behaviors.superCustomThemeMoleculeSliderField = {
    attach: (context) => {
      if (!Splide) {
        return;
      }
      ...
      once('m-slider-field', '.m-slider-field', context).forEach((el) => {
        ...
        if (sliderEl) {
          if (el.classList.contains('m-slider-field--type-1')) {
            behaviors.superCustomThemeMoleculeSliderField.typeOne(sliderEl);
          } else if (el.classList.contains('m-slider-field--type-2')) {
            behaviors.superCustomThemeMoleculeSliderField.typeTwo(sliderEl);
          } else {
            behaviors.superCustomThemeMoleculeSliderField.typeOne(sliderEl);
          }
        }
