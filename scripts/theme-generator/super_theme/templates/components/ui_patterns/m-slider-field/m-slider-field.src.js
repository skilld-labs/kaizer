...
(({ behaviors }, Splide) => {
  behaviors.superThemeMoleculeSliderField = {
    attach: (context) => {
      if (!Splide) {
        return;
      }
      ...
      once('m-slider-field', '.m-slider-field', context).forEach((el) => {
        ...
        if (sliderEl) {
          if (el.classList.contains('m-slider-field--type-1')) {
            behaviors.superThemeMoleculeSliderField.typeOne(sliderEl);
          } else if (el.classList.contains('m-slider-field--type-2')) {
            behaviors.superThemeMoleculeSliderField.typeTwo(sliderEl);
          } else {
            behaviors.superThemeMoleculeSliderField.typeOne(sliderEl);
          }
        }
