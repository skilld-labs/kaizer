...
(({ behaviors }, $) => {
  behaviors.superThemeHelperWrapperAsLink = {
    attach: (context) => {
      once('wrapper-as-link', '[wrapper-as-link-container]', context).forEach(
        (el) => {
          ...
          if (link) {
            behaviors.superThemeHelperWrapperAsLink.handler(el, link);
          }
        },
      );
    },
    handler: (el, link) => {
      ...
      if (isAjaxLink) {
        el.addEventListener('mousedown', (e) => {
          behaviors.superThemeHelperWrapperAsLink.fakeLinkEvents(
            el,
            link,
            e,
            isAjaxLink,
        ...
        el.addEventListener('click', (e) => {
          behaviors.superThemeHelperWrapperAsLink.fakeLinkEvents(el, link, e);
        });
      }
