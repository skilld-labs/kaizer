...
(({ behaviors }, $) => {
  behaviors.trololoHelperWrapperAsLink = {
    attach: (context) => {
      once('wrapper-as-link', '[wrapper-as-link-container]', context).forEach(
        (el) => {
          ...
          if (link) {
            behaviors.trololoHelperWrapperAsLink.handler(el, link);
          }
        },
      );
    },
    handler: (el, link) => {
      ...
      if (isAjaxLink) {
        el.addEventListener('mousedown', (e) => {
          behaviors.trololoHelperWrapperAsLink.fakeLinkEvents(
            el,
            link,
            e,
            isAjaxLink,
        ...
        el.addEventListener('click', (e) => {
          behaviors.trololoHelperWrapperAsLink.fakeLinkEvents(el, link, e);
        });
      }
